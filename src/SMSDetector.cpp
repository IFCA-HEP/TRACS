...
SMSDetector::SMSDetector(double pitch, double width, double depth, double nns, char bulk_type, char implant_type, int n_cells_x = 100, int n_cells_y = 100 )
{
  // set members
  _pitch = pitch;
  _width = width;
...
void SMSDetector::set_pde_variables()
{
  ...
  _L_g = new Gradient::FunctionSpace(_V_g);
...
void solve_w_u()
{
  ...
  // Set BC variables
  DirichletBC central_strip_BC(V, central_strip_V, _central_strip);
  DirichletBC neighbour_strip_BC(V, neighbour_strip_V, _neighbour_strip);
  DirichletBC backplane_BC(V, backplane_V, _backplane);
  // Collect them
